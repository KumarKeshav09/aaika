...
const TestimonialPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const testimonialData = { imageLink: uploadedImage ? [uploadedImage] : [] };

    console.log("Submitting testimonial data:", testimonialData);

    try {
      const response = await fetch(
        `${API_BASE_URL}/testimonial/createTestimonial`,
        {
          method: "POST",
      ...
      console.log("Testimonial creation response:", result);
      if (response.ok) {
        toast.success("Testimonial created");
        resetForm();
        router.push("/Testimonials")
      } else {
        toast.error("Testimonial creation failed:", result);
      }
    } catch (error) {
