...
const EditTestimonialPage = ({ params }) => {
  ...
  useEffect(() => {
    const fetchTestimonial = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/testimonial/getTestimonialById/${testimonialId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    const testimonialData = { imageLink: uploadedImage ? [uploadedImage] : [existingImage] };

    console.log("Submitting testimonial data:", testimonialData);

    try {
      const response = await fetch(
        `${API_BASE_URL}/testimonial/updateTestimonial/${testimonialId}`,
        {
          method: "PUT",
      ...
      if (response.ok) {
        ...
        toast.error("Testimonial update failed:", result);
      }
    } catch (error) {
