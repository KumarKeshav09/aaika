...
export default function HomeMenuAdmin() {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/menu/getAllHouseParties?page=${page}`
      );
    ...
    <section className="md:p-5">
      <div className="relative overflow-x-auto sm:rounded-lg">
        <div className="flex flex-col sm:flex-row flex-wrap space-y-4 sm:space-y-0 justify-end pb-4">
          <div>
            <Link href={"/Menus/HousePartiesAdmin/AddMenu"}>
              <button
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-nowrap">note</th>
                ...
                <tr key={item._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      <Link href={`/Menus/HousePartiesAdmin/${item._id}`} className="font-medium text-blue-600 text-lg dark:text-blue-500 hover:underline">
      ...
      <Pagination data={listData} pageNo={handlePageChange} pageVal={page} totalCount={listData?.total} />
      <Popup
        isOpen={isPopupOpen}
