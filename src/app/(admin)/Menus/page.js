  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/menu/getAllMenus?page=${page}`
      );
  ...
  return (
    <section className="md:p-5">
      <div className="relative overflow-x-auto sm:rounded-lg">
        <h1 className="text-2xl text-black underline mb-3 font-bold">Menus</h1>
                ...
                <tr key={item._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      <Link href={`/Menus/${item._id}`} className="font-medium text-blue-600 text-lg dark:text-blue-500 hover:underline">
      ...
      <Pagination data={listData} pageNo={handlePageChange} pageVal={page} totalCount={listData?.totalMenus} />
      <Popup
        isOpen={isPopupOpen}
